...
export class AppointmentsService {
  ...
  async create(createAppointmentDto: CreateAppointmentDto) {
    ...
    try {
      ...
      if (date <= new Date()) throw new BadRequestException(`La fecha de la cita no puede ser en el pasado`);
      ...
      return {
        ...
        data: { ...rest, date: dateInMexico, doctor: doctorClean, patient: patientClean }
      };
  ...
  async findAll(doctor_id?: number, start_date?: string, end_date?: string) {
    try {
      ...
      if (doctor_id) {
        await this.doctorsService.findOne(doctor_id);
      }
      ...
      if (start_date) {
        const start = new Date(`${start_date}T00:00:00.000-06:00`);
        query.andWhere('appointment.date >= :start_date', { start_date: start });
      }

      if (end_date) {
        const end = new Date(`${end_date}T23:59:59.999-06:00`);
  ...
  async cancel(id: number) {
    try {
      ...
      await this.appointmentRepository.save(appointment);
      const { createdAt, updatedAt, deletedAt, isActive, date, ...rest } = appointment;
      const dateInMexico = new Date(date).toLocaleString('es-MX', {
        timeZone: 'America/Mexico_City',
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
      ...
      return { message: `La cita con el id ${id} ha sido cancelada`, data: { date: dateInMexico, ...rest, } };
